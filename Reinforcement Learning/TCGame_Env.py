...
class TicTacToe():
    ...
    def is_winning(self, curr_state):
        ...
        # Check whether the rows sum up to 15
        for i in range(3):
            if (curr_state[i * 3] + curr_state[i * 3 + 1] + curr_state[i * 3 + 2]) == 15:
                return True
        
        # Check whether the columns sum up to 15
        for i in range(3):
            if (curr_state[i + 0] + curr_state[i + 3] + curr_state[i + 6]) == 15:
                return True
        
        # Check whether the diagonals sum up 15
    ...
    def step(self, curr_state, curr_action):
        ...
        # Check whether the game ends with a win or tie after the move of agent.
        if is_win_tie[0]:
            if is_win_tie[1] == 'Win': #Win
                reward = 10
            else:
                reward = 0 #Tie
        ...
        # The game is not yet finished. The next will be a random move from the environment action space
        ...
        # Check whether the game ends with a win or tie after environment
        is_win_tie = self.is_terminal()
        
        if is_win_tie[0]:
            if is_win_tie[1] == 'Win':
                reward = -10 # Environment win
            else:
                reward = 0 # Tie
